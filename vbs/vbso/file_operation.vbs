...
with createobject( "scripting.filesystemobject" )
    ...
    set file = .createtextfile( path, true ) ' true: file can be overwritten
end with
...
' write/read stream
const write = 2
set stream = fso.opentextfile( path, write, false, 0 ) ' false: file needs to be exist
...
const append = 8
set stream = fso.opentextfile( path, append, false, 0 )
stream.writeline( 789 )
stream.close( )
...
' create employee record (name/department)
